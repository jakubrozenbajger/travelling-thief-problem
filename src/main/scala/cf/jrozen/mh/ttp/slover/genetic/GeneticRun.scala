...
object GeneticRun extends App {
  ...
  val params = GeneticParameters(populationSize = 100, noOfGenerations = 200, tournamentSize = 5, mutationProbability = 0.022, crossoverProbability = 0.119)
//
//  val problemName = "medium_2"
//  val stats = solve(problemName, params)
//  System.out.println("Last stats: " + stats.last)
//  chart(params).show(stats)


  val problemNames = List("easy_1", "easy_4", "medium_0", "medium_1")
  ...
  private def solve(problemName: String, params: GeneticParameters) = {
    ...
    implicit val saParameters = SimulatedAnnealingParameters(iterations = 6000, startingTemperature = 200.0, coolingRate = 0.001, stopTemperature = 0.0000001)
...
//    val evolutionHistory = Population.initRandom(context, params, new SAMutationStrategy(new GAMutationStrategy(context, params), rnd.nextFloat() < 0.0091210 ), EmptyFinish()).runEvolution.asJava().toList
//    val evolutionHistory = Population.initRandom(context, params, new SAMutationStrategy(new GAMutationStrategy(context, params), rnd.nextFloat() < 0.0070 ), TabuFinish()).runEvolution.asJava().toList
  ...
  def runFor(problemName: String, parameters: GeneticParameters, times: Int = 3): (String, Stats) = {
